...
int** uniqueSorted(int *A, int length, int *quantity) {
    int **temp = new int*[length], **result = NULL;
    int i, lastValue = 0;
    ...
    if (i >= length && *free_jokers > 0) {
        ...
        if (temp <= 1000000) {
            ...
            *free_jokers = *free_jokers - temp;
            ...
            temp = MIN(*free_jokers, *A[*start_index] - 1);
            *free_jokers = *free_jokers - temp;
            quantity += temp;
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    if (argc >= 2) {
        ...
        //input_file = "/Users/red_lion/Documents/Projects/GitHub/algorithms-lits/lngpok/inputData/01.in";
    ...
    if (quantity - joker_quantity > 0) {
        ...
        while (end_index < sorted_uniqued_cards_quantity) {
            ...
            if (joker_quantity > 0 && next_start_index > 0 && next_start_index < sorted_uniqued_cards_quantity) {
                if (end_index == next_start_index) {
                    ...
                    free_joker_quantuty = sequence_length - (*sorted_uniqued_cards[next_start_index - 1] - *sorted_uniqued_cards[start_index] + 1) - (*sorted_uniqued_cards[end_index] - *sorted_uniqued_cards[next_start_index] + 1);
                }
            }
            ...
            if (end_index == next_start_index) {
                ...
                sequence_length = *sorted_uniqued_cards[end_index] - *sorted_uniqued_cards[next_start_index] + 1;
            }

            start_index = next_start_index;
        }
    } else {
        max_sequence_length = MIN(joker_quantity, 10000);
    }
